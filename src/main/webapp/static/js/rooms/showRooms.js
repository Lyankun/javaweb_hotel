layui.use(['jquery','layer', 'table','form','laydate','upload'], function() {
    var $ = layui.jquery    //引入jquery模块
        ...
        ,upload=layui.upload;//引入上传模块
  //layer.msg("测试！")

    //七牛云的存储空间的域名
    var qnyName = "http://qgddj7o5v.hn-bkt.clouddn.com/";
...
//初始化所有的客房类型数据
    ...
    var uploadInst = upload.render({
        ...
        ,done: function(res){  //执行服务器上传后的函数回调
            ...
            if(res.code > 0){
                ...
                //将目标文件的文件名填充到roomPicId的input中去，最后添加时提交到数据库去
                //无论用户上传多少次，都是保存最后一次的文件名字，前面的都覆盖掉了
    ...
    function  loadAllRooms(){
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常1！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
    ...
    function updRoomsFlag(roomid,flag) {
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常2！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
    ...
    function updRoomStatus(roomid,roomStatus) {
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常3！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
...
//加载所有的客房类型数据
    function loadAllRoomType() {
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常4！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
...
//自定义验证
    ...
    function checkRoomNum(roomNum) {
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常5！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
...
//执行添加
    function saveRooms(saveJsonRooms) {
        $.ajax({
            ...
            error:function () {
                layer.msg("服务器异常6！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
            }
        });
    }
