layui.use(['jquery','layer', 'table','form','laydate'], function() {
    ...
    table.render({  //数据表格的数据渲染(此UI框架底层是进行异步加载)
        ...
        , cols: [[ //表头
            ...
            , {field: 'roomPic', title: '封面图', align: 'center', width: 120, sort: true, templet: "<div><img src='http://qgddj7o5v.hn-bkt.clouddn.com/{{d.rooms.roomPic}}'></div>"}
    ...
    table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            ...
            form.val("exitInRoomInfoForm", { //formTest 即 class="layui-form" 所在元素属性 lay-filter="" 对应的值
             ...
             //1.3回显退房时间
            var endDateStr= getNowDate(new Date()) ;
             $("#endDate").val(endDateStr);

            //1.4计算入住天数
            //看下面时间转换格式
           var days = getDays(getDateStr(data.createDate),getDateStr(endDateStr));
           //因为span元素用text
           $("#days").text(days);

           //1.5计算出房费（单价*天数*折扣）
            var vipCount=parseFloat($("#vipRate").val());//折扣 async:false,才可以获取折扣
            var zprice=data.rooms.roomType.roomPrice*days*vipCount;//房费  不含其他消费
            $("#zprice").text(zprice);//回显住房金额

            //2弹页面
            layer.open({
                type: 1,  //弹出类型
                title: "顾客修改界面",  //弹框标题
                area: ['680px', '600px'],  //弹框款高度
                anim: 4,  //弹出的动画效果
                shade: 0.5,  //阴影遮罩
                content: $("#exitInRoomInfoDiv")  //弹出的内容
            });
            //3计算消费总金额
            //3.1输入其他消费计算总金额 zprice+otherprice  只有事件才能触发函数给它一个失去焦点事件
               $("#otherPrice").blur(function () {
                   var salePrice = zprice +parseFloat($("#otherPrice").val());
                   $("#zprice").text(salePrice);//回显总金额
               });
            form.on('submit(demo3)',function(data){
                var saveOrdersJson = {};  //定义订单添加的数据
                var nowDateStr=getNowDate(new Date());//获取当前时间字符串yyyy/MM/dd HH:mm:ss 格式的字符串
        ...
        function updRoomStatus(id,obj) {
            ...
            error:function () {
                layer.msg("服务器异常3！！！",{icon: 6,time:2000,anim: 6,shade:0.5});
            }
        });
    }
    ...
    function loadVipByIdCard(idcard) {
        $.ajax({
            ...
            success:function (data) {
                ...
                $("#vipRate").val(data.vipRate);    //在页面中被隐藏了会员卡折扣
            },
            error:function () {
                layer.msg("服务器异常1！！！",{icon: 6,time:2000,anim: 6,shade:0.5});
            }
        });
    }
   //退房
