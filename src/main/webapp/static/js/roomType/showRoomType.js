...
/***************************************监听折叠面板房间数据**********************************************************/
    ...
    function checkRoomsOfRoomType(id) {
        $.ajax({
            ...
            error: function () {
                layer.msg("服务器异常2！！！", {icon: 6, time: 2000, anim: 6, shade: 0.5});
            }
        });
    }
    ...
    function delRoomTypeById(id) {
        $.ajax({
            type: 'POST',
            url: 'roomType/delByPrimaryKey',  //调用的是base系列的方法，只需要改mapper.xml文件
            data:{"id":id},
            success: function (data) {  //data为服务器端的map集合数据
                if(data=='success'){
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data==1){
                    ...
                    checkRoomTypeNameIf = false;  //有重复的房间名，验证不通过
                }else {
                    layer.tips('没有重复的房型名称，验证通过','#roomTypeName', {tips: [2,'green'],time:2000});
                    checkRoomTypeNameIf = true;  //没有重复的房间名，验证通过
                }
            },
            error:function () {
